	...
	public static JSONArray translateCopernicusMetadata (String results, String source) {
		...
		if(source.equals("dhus")) {
			...
			for (int i=0; i<entry.size(); i++) {
				...
				product.put("productURL", "https://scihub.copernicus.eu/dhus/odata/v1/Products('"+entry.get(i).getAsJsonObject().get("id").getAsString()+"')/");
				for(int j=0; j<entry.get(i).getAsJsonObject().getAsJsonArray("date").size(); j++) {
					if(entry.get(i).getAsJsonObject().getAsJsonArray("date").get(j).getAsJsonObject().get("name").getAsString().equals("beginposition")) {
						product.put("date", entry.get(i).getAsJsonObject().getAsJsonArray("date").get(j).getAsJsonObject().get("content").getAsString());
					}
				}
				for(int j=0; j<entry.get(i).getAsJsonObject().getAsJsonArray("str").size(); j++) {
					if(entry.get(i).getAsJsonObject().getAsJsonArray("str").get(j).getAsJsonObject().get("name").getAsString().equals("footprint")) {
						product.put("location", entry.get(i).getAsJsonObject().getAsJsonArray("str").get(j).getAsJsonObject().get("content").getAsString());
					}
				}	
			...
			for (int i=0; i<entry.size(); i++) {
				...
				product = new JSONObject();
				product.put("id", entry.get(i).getAsJsonObject().get("id").getAsString());			
				product.put("productURL", "https://catalogue.onda-dias.eu/dias-catalogue/Products("+entry.get(i).getAsJsonObject().get("id").getAsString()+")");
